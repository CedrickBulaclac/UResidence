...
namespace UResidence
{
    public class ReportReservationAmenityController
    {
        ...
        {
            //const string GET_ALL = @"select t.BldgNo,t.UnitNo,ISNULL((t.Fname+' '+t.Mname+' '+t.Lname),0) as OwnerName,rf.Id as RefNo,StartTime,EndTIme,Convert(varchar(25),StartTime,100)+' - '+RIGHT(Convert(varchar,EndTIme,100),7) as DateofReservation,a.AmenityName,a.Rate,a.EveningRate,DATEDIFF(HOUR,StartTime,EndTIme) as AmenityHour,s.Rate as TotalAmenityRate,s.Date,TypeResident,Charge =(select ISNULL(SUM(Charge),0) as Charge from tbCharge where RefNo=rf.Id),rc.Totalpayment,rc.Description,rc.Date as DateP,TotalEquipmentRate=(select ISNULL(SUM(er.Rate),0) from tbEquipReservation er where er.RefNo=rf.Id) from tbSchedReservation s inner join tbReservationForm  rf on rf.SchedId=s.Id inner join tbResidence r on r.Id=rf.ResidentId full join tbTenant t on t.Id=r.TenantNo inner join tbAmenity a on a.Id=s.AmenityId inner join tbReceipt rc on rf.Id=rc.RefNo where rf.Id=@refid";
